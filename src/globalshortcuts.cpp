                                          ...
                                          QVariantMap &replyResults)
{
    ...
    delayReply(message, dialog, session, [model, returningShortcutInfos, session](DialogResult result) -> QVariantList {
        ...
        // Still return a cancelled result if there are no active shortcuts.
...
void GlobalShortcutsSession::setActions(const QList<ShortcutInfo> &shortcuts)
{
    ...
    for (const auto &shortcut : shortcuts) {
        if (shortcut.id.isEmpty()) {
            qCWarning(XdgDesktopPortalKdeGlobalShortcuts) << "Shortcut without id" << "for" << componentName();
            continue;
        }
        if (shortcut.description.isEmpty()) {
            qCWarning(XdgDesktopPortalKdeGlobalShortcuts) << "Shortcut without name or description" << shortcut.id << "for" << componentName();
            continue;
        }
        ...
        const QString app = Utils::applicationName(m_appId);
        action->setProperty("componentName", componentName());
        action->setProperty("componentDisplayName", app.isEmpty() ? componentName() : app);
