                               ...
                               [[maybe_unused]] QVariantMap &replyResults)
{
    ...
    if (persistMode != PersistMode::None && restoreData.session == "KDE"_L1 && restoreData.version == RestoreData::currentRestoreDataVersion()) {
        ...
        if (canRestoreCapabilities && canRestoreClipboard) {
            ...
            auto notification = new KNotification(QStringLiteral("inputcapturesrestored"), KNotification::CloseOnTimeout);
            notification->setTitle(i18nc("@title:nofication", "Input capture session started"));
            QString description = applicationName.isEmpty() ? i18nc("@info", "An application has started an input capture session.\n")
                                                            : i18nc("@info", "%1 has started an input capture session.", applicationName);
