...
{
    return globalWaylandIntegration->startStreamingOutput(outputName);
}

bool WaylandIntegration::startStreamingWindow(quint32 winid)
{
...
bool WaylandIntegration::WaylandIntegrationPrivate::startStreamingWindow(quint32 winid)
{
    return startStreaming(m_screencasting->createWindowStream(winid), {});
}

bool WaylandIntegration::WaylandIntegrationPrivate::startStreamingOutput(quint32 outputName)
{
    auto output = m_outputMap.value(outputName).output();

    return startStreaming(m_screencasting->createOutputStream(output.data()), output->globalPosition());
}

bool WaylandIntegration::WaylandIntegrationPrivate::startStreaming(KWayland::Client::ScreencastingStream *stream, const QPoint &globalPosition)
{
    QEventLoop loop;
    bool streamReady = false;
    connect(stream, &KWayland::Client::ScreencastingStream::failed, this, [&] (const QString &error) {
    ...
    connect(stream, &KWayland::Client::ScreencastingStream::created, this, [&] (uint32_t nodeid, const QSize &pixelSize) {
        m_streamedScreenPosition = globalPosition;
        Stream s;
        s.stream = stream;
        s.nodeId = nodeid;
        s.map = {{QLatin1String("size"), pixelSize}};
        m_streams.append(s);
        startStreamingInput();

        qDebug() << "wop!";
        connect(stream, &KWayland::Client::ScreencastingStream::closed, this, [this, nodeid] { stopStreaming(nodeid); });
        streamReady = true;
        loop.quit();
    });
...
void WaylandIntegration::WaylandIntegrationPrivate::setupRegistry()
{
    ...
    connect(m_registry, &KWayland::Client::Registry::interfaceAnnounced, this, [this] (const QByteArray &interfaceName, quint32 name, quint32 version) {
        if (interfaceName != "zkde_screencast_unstable_v1")
            return;
        m_screencasting = new KWayland::Client::Screencasting(m_registry, name, version, this);
    });
