...
{
    return u"/org/kde/KWin/EIS/RemoteDesktop"_s;
}
                                        ...
                                        QVariantMap &results)
{
    ...
    connect(session, &Session::closed, [session] {
        ...
        if (remoteDesktopSession->eisCookie()) {
            auto msg = QDBusMessage::createMethodCall(kwinService(), kwinRemoteDesktopPath(), kwinRemoteDesktopInterface(), u"disconnect"_s);
...
RemoteDesktopPortal::ConnectToEIS(const QDBusObjectPath &session_handle, const QString &app_id, const QVariantMap &options, const QDBusMessage &message)
{
    ...
    auto msg = QDBusMessage::createMethodCall(kwinService(), kwinRemoteDesktopPath(), kwinRemoteDesktopInterface(), u"connectToEIS"_s);
    ...
    if (reply.isError()) {
        ...
        auto error = message.createErrorReply(QDBusError::Failed, "Failed to connect to to EIS");
