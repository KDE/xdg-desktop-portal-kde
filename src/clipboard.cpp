...
QByteArray ClipboardPortal::fetchData(Session *session, const QString &mimetype)
{
    static int transferSerialCounter = 0;
    const int transferSerial = transferSerialCounter++;
    // This whole thing sucks big time (no async QMimeData once more), better implement data control here directly
...
QDBusUnixFileDescriptor ClipboardPortal::SelectionWrite(const QDBusObjectPath &session_handle, uint serial, const QDBusMessage &message)
{
    ...
    connect(new QSocketNotifier(pipeFds[0], QSocketNotifier::Read, &transfer->second.loop), &QSocketNotifier::activated, [transfer] {
        ...
        while (true) {
            ...
            if (bytesRead < 0 && bytesRead && errno != EAGAIN) {
                qCWarning(XdgDesktopPortalKdeClipboard) << "Error reading" << strerror(errno);
                transfer->second.data.clear();
                transfer->second.loop.quit();
                break;
            } else if (bytesRead == 0) {
                break;
            } else {
...
void ClipboardPortal::SelectionWriteDone(const QDBusObjectPath &session_handle, uint serial, bool success)
{
    ...
    auto transfer = m_pendingTransfers.find(serial);
    if (transfer == m_pendingTransfers.end()) {
        qCWarning(XdgDesktopPortalKdeClipboard) << "No pending transfer with this serial";
        return;
    }
