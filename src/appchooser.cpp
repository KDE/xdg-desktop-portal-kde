                                         ...
                                         [[maybe_unused]] QVariantMap &results)
{
    ...
    connect(appDialog, &QuickDialog::finished, this, [this, message, appDialog, handle](QuickDialog::Result result) {
        QVariantMap results;
                                                ...
                                                [[maybe_unused]] QVariantMap &results)
{
    ...
    auto appDialog =
        new AppChooserDialog({}, options.value(QStringLiteral("last_choice")).toString(), itemName, options.value(QStringLiteral("content_type")).toString());
    Utils::setParentWindow(appDialog->windowHandle(), parent_window);
    ...
    connect(appDialog, &QuickDialog::finished, this, [msg, appDialog](QuickDialog::Result result) {
        QVariantMap results;
        if (result == QuickDialog::Result::Accepted) {
        ...
        const auto reply = msg.createReply({qToUnderlying(result), results});
        QDBusConnection::sessionBus().send(reply);
    });
    return 0;
}
