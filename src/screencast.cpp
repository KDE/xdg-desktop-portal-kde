...
void continueStartAfterDialog(ScreenCastSession *session,
                              ...
                              bool allowRestore)
{
    ...
    auto sendReply = [call](uint response, const QVariantMap &results = {}) {
        auto reply = call.createReply({response, results});
                             ...
                             [[maybe_unused]] QVariantMap &results)
{
    ...
    if (valid) {
        continueStartAfterDialog(session, message, selectedOutputs, selectedRegion, selectedWindows, true);
        return;
    }
    ...
    connect(screenDialog, &QuickDialog::finished, this, [message, screenDialog, session](QuickDialog::Result result) {
        if (result == QuickDialog::Result::Rejected) {
            const auto reply = message.createReply(qToUnderlying(result));
            QDBusConnection::sessionBus().send(reply);
            return;
        }
