                                               ...
                                               bool allowRestore)
{
    ...
    QList<QFuture<WaylandIntegration::Stream>> streams;
    ...
    for (const auto &output : std::as_const(selectedOutputs)) {
        QFuture<WaylandIntegration::Stream> stream;
        ...
        streams << stream.onCanceled([output] {
            ...
            return WaylandIntegration::Stream();
        });
    ...
    return all
        .then(session,
              [session, allowRestore, outputs, windows, selectedRegion](const QList<QFuture<WaylandIntegration::Stream>> &result) -> QVariantList {
                  QVariantMap results;
                  WaylandIntegration::Streams streams;
                  for (const auto &stream : result) {
                      if (stream.isCanceled() || !stream.result().isValid()) {
                          for (const auto &stream : result) {
                              if (!stream.isCanceled() && stream.result().isValid()) {
                                  stream.result().close();
                              }
                      ...
                      streams.push_back(stream.result());
                  }
                             ...
                             [[maybe_unused]] QVariantMap &replyResults)
{
    ...
    if (valid) {
        auto result = continueStartAfterDialog(session, selectedOutputs, selectedRegion, selectedWindows, true);
        delayReply(message, result, this);
        return;
    }
    ...
    auto result = dialogDone.then([screenDialog, session](DialogResult result) {
    ...
    delayReply(message, result.unwrap(), this);
}
